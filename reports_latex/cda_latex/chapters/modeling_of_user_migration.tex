...
\label{sec:ml_migration}

In this chapter\footnote{The complete experimental pipeline and visualization tools are available in the project
repository: \texttt{project\_cda/4\_user\_migration\_pipeline.ipynb}.}, we address the problem of user migration between communities,
...
The primary advantage of Leiden over Louvain lies in its handling of community connectivity \cite{leiden}, scalability and reliability on large networks.
...
We utilized \textbf{CatBoost} \cite{catboost}, a gradient boosting algorithm on decision trees, for the classification task. CatBoost was selected for its
...
Demographically, \textbf{older} users show lower migration propensities.
